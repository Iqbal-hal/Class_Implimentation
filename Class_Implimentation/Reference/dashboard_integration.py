...
class TradingDashboard:
    ...
    def prepare_dashboard_data(self):
        ...
        """
        print("ðŸ”„ Preparing data for visualization dashboard...")
        
        stock_data = {}
        transaction_data = {}
        
        if self.backtested_scrips_df.empty:
            print("âš ï¸ No backtested scrip data available")
            return
        ...
        for stock in unique_stocks:
            ...
            for idx, row in stock_df.iterrows():
                try:
                    ...
                    print(f"âš ï¸ Error processing row for {stock}: {e}")
                    continue
            ...
            if not self.backtested_transactions_df.empty:
                ...
                for idx, row in stock_transactions.iterrows():
                    try:
                        ...
                        print(f"âš ï¸ Error processing transaction for {stock}: {e}")
                        continue
        ...
        print(f"âœ… Dashboard data prepared for {len(unique_stocks)} stocks")
    ...
    def save_data_for_dashboard(self, filename='dashboard_data.json'):
        ...
        try:
            ...
            print(f"âŒ Error saving dashboard data: {e}")
            return None
...
<body>
    <div class="header">
        <h1>ðŸ“Š Trading Portfolio Dashboard</h1>
        <p>Interactive Candlestick Charts with Buy/Sell Signals</p>
    </div>

    <div class="container">
        <div class="real-data-banner">
            ðŸŽ¯ <strong>LIVE TRADING DATA</strong> - Generated from your actual backtesting results
        </div>
        
        <div class="controls">
            <h3>ðŸ“ˆ Chart Controls</h3>
            <div class="control-group">
                ...
                <div class="control-item">
                    <button onclick="generateChart()">ðŸ“Š Generate Chart</button>
                </div>
            </div>
        </div>
        ...
        <div id="portfolioStats" class="stats-grid" style="display: none;">
            ...
            <div class="stat-card">
                <div class="stat-value positive" id="totalPnL">â‚¹0</div>
    ...
    <script>
            ...
            data.forEach(d => {
                if (d.buySignal) {
                    ...
                    buyTexts.push(transaction ? 
                        `BUY: ${transaction.quantity} shares<br>Price: â‚¹${transaction.price}<br>Amount: â‚¹${transaction.amount.toLocaleString()}` :
                        `BUY Signal<br>Price: â‚¹${d.close.toFixed(2)}`
                    );
                }
                if (d.sellSignal) {
                    ...
                    sellTexts.push(transaction ? 
                        `SELL: ${transaction.quantity} shares<br>Price: â‚¹${transaction.price}<br>Amount: â‚¹${transaction.amount.toLocaleString()}<br>P&L: â‚¹${transaction.pnl.toLocaleString()}<br>Holding: ${transaction.holdingDays} days` :
                        `SELL Signal<br>Price: â‚¹${d.close.toFixed(2)}`
                    );
                }
            });
        ...
        function updatePortfolioStats(transactions) {
            ...
            document.getElementById('totalPnL').textContent = `â‚¹${totalPnL.toLocaleString()}`;
            ...
            transactions.slice(-20).reverse().forEach(transaction => {
                ...
                row.innerHTML = `
                    ...
                    <td class="${transaction.pnl >= 0 ? 'positive' : 'negative'}">
                        ${transaction.pnl !== 0 ? `â‚¹${transaction.pnl.toLocaleString()}` : '-'}
                    </td>
    ...
    def launch_dashboard(self, filename='trading_dashboard.html', auto_open=True):
        ...
        """
        print("ðŸš€ Launching Trading Dashboard...")
        ...
        try:
            ...
            print(f"âœ… Dashboard created: {file_path}")
            ...
            print(f"âŒ Error launching dashboard: {e}")
            return None
    ...
    def export_dashboard_data(self, export_dir='dashboard_exports'):
        ...
        try:
            ...
            print(f"âœ… Dashboard exported to '{export_dir}' directory")
            ...
            print(f"âŒ Error exporting dashboard: {e}")
            return None
...
def add_dashboard_method():
    ...
    def create_dashboard(self, launch_browser=True):
        ...
        if len(self.backtested_scrip_df_list) == 0:
            print("âŒ No backtested data available. Run backtesting first.")
            return None
...
def enhanced_run_method():
    ...
    def run(self, master_df, create_dashboard=True):
        ...
        print(f"â±ï¸  Min Holding Period: {config.MIN_HOLDING_PERIOD} days")
        print(f"ðŸŽ¯ Min Profit Target: {config.MIN_PROFIT_PERCENTAGE}%")
        ...
        if create_dashboard and not backtested_scrips_df.empty:
            ...
            try:
                ...
                if dashboard_file:
                    print(f"ðŸ“Š Interactive Dashboard: {dashboard_file}")
                    print("ðŸŽ¯ Features Available:")
                    print("   â€¢ Interactive candlestick charts")
                    print("   â€¢ Buy/Sell signal markers with details")
                    print("   â€¢ Transaction history tables")
                    print("   â€¢ Portfolio performance statistics")
                    print("   â€¢ Zoom, pan, and annotation tools")
                ...
                print("ðŸ“ˆ Trading analysis completed without visualization")
...
if __name__ == "__main__":
    ...
    â€¢ Interactive candlestick charts with OHLC data
    â€¢ Buy/Sell signals marked as triangles on price timeline  
    â€¢ Hover tooltips showing trade details (quantity, price, P&L)
    â€¢ Portfolio statistics (win rate, total trades, avg holding period)
    â€¢ Transaction history table with filtering
    â€¢ Volume overlay option
    â€¢ Date range filtering
    â€¢ Zoom, pan, drawing tools
    â€¢ Export capabilities (PNG, HTML, JSON)
    
    ðŸ’¡ CUSTOMIZATION OPTIONS:
    â€¢ Add technical indicators overlay
    â€¢ Custom color schemes for different strategies
    â€¢ Multi-timeframe analysis
    â€¢ Portfolio comparison tools
    â€¢ Risk metrics visualization
    """)
    
    print("\nâœ… Dashboard integration module ready!")
    print("ðŸ“ Save this as 'dashboard_integration.py' alongside your main script")
    print("ðŸš€ Import and integrate with your FilteringAndBacktesting class")

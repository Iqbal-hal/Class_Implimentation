...
class FilteringAndBacktesting:
    ...
    def backtested_global_summary(self, backtested_scrips_df, backtested_transactions_df, master_df=None):
        ...
        if not backtested_transactions_df.empty:
            ...
            if 'Date' in tx_df.columns:
                ...
                if len(date_col_indices) > 1:
                    ...
                    # if Date exists but is not a Series (rare), coerce to string
                    if not isinstance(tx_df['Date'], pd.Series):
                        tx_df['Date'] = tx_df['Date'].astype(str)

            # Now safe to parse to datetime
        ...
        if not backtested_transactions_df.empty and 'Date' in backtested_transactions_df.columns:
            try:
                start_date = pd.to_datetime(backtested_transactions_df['Date'], dayfirst=True, errors='coerce').min()
            except Exception:
                start_date = None
        ...
        with pd.ExcelWriter("portfolio_performance_report.xlsx", engine="openpyxl") as writer:
            ...
            if not backtested_transactions_df.empty:
                ...
                # Ensure there's a single 'Date' column (combine duplicates if present)
                if 'Date' in tx_export.columns:
                    date_col_indices = [i for i, c in enumerate(tx_export.columns) if c == 'Date']
                    if len(date_col_indices) > 1:
                        date_df = tx_export.iloc[:, date_col_indices]
                        # take first non-null across duplicate Date columns
                        combined_dates = date_df.bfill(axis=1).iloc[:, 0]
                        tx_export = tx_export.drop(columns=[c for c in tx_export.columns if c == 'Date'])
                        tx_export['Date'] = combined_dates.values
                    else:
                        # if Date exists but is not a Series (rare), coerce to string
                        if not isinstance(tx_export['Date'], pd.Series):
                            tx_export['Date'] = tx_export['Date'].astype(str)
